class ReviewsController < ActionController::Base
    ...
    def show
        render json: Review.eager_load(:review_likes, comments: [:user, :likes]).find(params[:id]).as_json(include: [:review_likes, comments: {include: [:user, :likes]}])
    end
    ...
    def update
        ...
        if check_admin? && review.update(review_params.reject{|_, v| v.blank?})
             render json: { status: 'ok'}
        else
            render json: { errors: review.errors.full_messages }, status: :unprocessable_entity
        end
    ...
    def destroy
        review = Review.find_by_id(params[:id])
          ...
          review.review_likes.each{|x| x.delete}
          render json: { status: 'ok'}
          else
          render json: { errors: review.errors.full_messages }, status: :unprocessable_entity
          end
